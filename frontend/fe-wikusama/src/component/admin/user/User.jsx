...
function SearchInput({ onSearch, onClearSearch }) {
  ...
  return (
    <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between">
      {/* <input
        ...
        className="w-full sm:w-32 py-2 pl-10 ml-10 mt-5 text-sm rounded-md sm:w-auto focus:outline-none text-white bg-gray-200"
        value={searchTerm}
        onChange={(e) => setSearchTerm(e.target.value)}
      />
      <button
        type="button"
        className="mb-1 text-white focus:ring-4 focus:outline-none font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center bg-green-500 hover:bg-blue-800 focus:ring-blue-900"
      ...
      <button
        type="button"
        className="mt-2 text-sm text-gray-500 underline hover:text-gray-800 focus:outline-none"
...
export default function Menu() {
  ...
  useEffect(() => {
    const fetchAllUser = async () => {
      try {
        const res = await axios.get("http://localhost:8000/user/get", { headers });
  ...
  return (
    <div>
      <div className="my-8 mx-16 flex items-center justify-between">
        <div>
          <a href="add_user">
            <button type="button" className="mb-1 text-white focus:ring-4 focus:outline-none font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center bg-green-500 hover:bg-blue-800 focus:ring-blue-900">
              Tambahkan User
            </button>
          </a>
        </div>
